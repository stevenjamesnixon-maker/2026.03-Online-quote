...
- **£ symbol on upgrade price** — The Design+ upgrade price in the UFH Standard Design banner
  now displays with a `£` prefix (e.g. "£100.00 plus VAT"). Applied conditionally — if the
  value in `custbody_upgrades_itemprice` already begins with `£`, it is used as-is to prevent
  doubling. All styling from v4.3.66 is preserved unchanged.

### Files Changed
- `nuheat_quote_suitelet.js` — Price span in `renderDesignPackageCard()` updated with
  conditional `£` prefix; version bumped to v4.3.67
