...
- v4.3.56: Replaced `THERMOSTAT_OPTION_ITEM_IDS` exact matching with `THERMOSTAT_OPTION_PREFIXES` prefix matching. New thermostat variants now appear automatically without script changes. Sort updated: recommended first, then alphabetical by product name. ⏳ Pending Sandbox testing sign-off.
